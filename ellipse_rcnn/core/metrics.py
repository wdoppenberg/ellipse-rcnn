...
def mv_kullback_leibler_divergence(
    A1: torch.Tensor, A2: torch.Tensor, shape_only: bool = False
) -> torch.Tensor:
    ...
    if shape_only:
        ...
        displacement_term = (
            (m1 - m2).transpose(-1, -2) @ cov1.inverse() @ (m1 - m2)
        ).squeeze()

    return 0.5 * (trace_term + displacement_term - 2 + log_term)


def norm_mv_kullback_leibler_divergence(
    A1: torch.Tensor, A2: torch.Tensor
) -> torch.Tensor:
    return 1 - torch.exp(-mv_kullback_leibler_divergence(A1, A2))


def gaussian_angle_distance(
    A1: Union[torch.Tensor, np.ndarray], A2: Union[torch.Tensor, np.ndarray]
) -> torch.Tensor:
    A1, A2 = map(unimodular_matrix, (A1, A2))
    cov1, cov2 = map(lambda arr: -arr[..., :2, :2], (A1, A2))
    ...
    if isinstance(cov1, torch.Tensor) and isinstance(cov2, torch.Tensor):
        m1, m2 = map(
            lambda arr: torch.vstack(tuple(conic_center(arr))).T[..., None], (A1, A2)
        )
