...
def preprocess_label_files(root_path: str) -> Dict[str, List[List[float]]]:
    ...
    ellipse_dict: Dict[str, List[List[float]]] = {str(k): [] for k in pdf_file_paths["path"]}
...
class FDDB(EllipseDatasetBase):
    ...
    def load_target_dict(self, index: int) -> TargetDict:
        ...
        a = torch.tensor([e[0] for e in ellipses_list])
        b = torch.tensor([e[1] for e in ellipses_list])
        theta = torch.tensor([e[2] for e in ellipses_list])
        x = torch.tensor([e[3] for e in ellipses_list])
        y = torch.tensor([e[4] for e in ellipses_list])
