...
def test_adjugate_matrix() -> None:
    ...
    expected_output = torch.tensor([[12.0, -51.0, 21.0], [-4.0, -1.0, 2.0], [-1.0, 20.0, -13.0]], dtype=torch.float32)

    output = adjugate_matrix(input_matrix)

    assert torch.allclose(output, expected_output)

    input_matrices = torch.cat((input_matrix[None, ...], input_matrix[None, ...]), dim=0)

    expected_outputs = torch.cat((expected_output[None, ...], expected_output[None, ...]), dim=0)
...
def test_unimodular_matrix() -> None:
    ...
    """
    input_matrix = torch.tensor([[-3, 2, -5], [-5, 12, -1], [4, -6, 2]], dtype=torch.float32)

    assert torch.isclose(torch.det(unimodular_matrix(input_matrix)), torch.tensor(1.0))

    input_matrices = torch.cat((input_matrix[None, ...], input_matrix[None, ...]), dim=0)

    assert torch.allclose(torch.det(unimodular_matrix(input_matrices)), torch.tensor(1.0))
...
def test_ellipse_conic_conversion() -> None:
    ...
    assert torch.allclose(torch.cat(ellipse_axes(conic_matrices)), torch.cat((major_axis, minor_axis)))
